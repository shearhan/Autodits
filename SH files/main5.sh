...
echo -e "|4.2.2.5  | Ensure logging is configured (Manual)                                                                          |${BLUE} Not-Accessed     ${NC}|"
    ...
    grep '$ModLoad imtcp' /etc/rsyslog.conf /etc/rsyslog.d/*.conf >> output.txt
output=output.txt
if [ $(wc -l < "${output}") -eq 0 ]; then
        grep '$InputTCPServerRun' /etc/rsyslog.conf /etc/rsyslog.d/*.conf >> output.txt
        output=output.txt
        if [ $(wc -l < "${output}") -eq 0 ]; then

                echo -e "|4.2.2.7  |Ensure rsyslog is not configured to receive logs from a remote client (Automated)(Old-Format)                   |${green} PASS            ${NC} |"
                ((pass_count++))
                tput init
                else
                echo -e "|${red}4.2.2.7  |Ensure rsyslog is not configured to receive logs from a remote client (Automated)(Old-Format)                   | FAIL             ${NC}|"
                tput init
                ((fail_count++))
...
if [ $(wc -l < "${output}") -eq 0 ]; then
        ...
        if [ $(wc -l < "${output}") -eq 0 ]; then
                ...
                echo -e "|4.2.2.7  |Ensure rsyslog is not configured to receive logs from a remote client (Automated)(New-Format)                   |${green} PASS            ${NC} |"
                ((pass_count++))
                tput init
                else
                echo -e "|${red}4.2.2.7  |Ensure rsyslog is not configured to receive logs from a remote client (Automated)(New-Format)                   | FAIL             ${NC}|"
                tput init
                ((fail_count++))
                failed_sections+="4.2.2.7 "
        fi
